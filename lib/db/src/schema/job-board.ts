 ...
 */
export const jobSourcesTable = pgTable(
  "job_sources",
  {
...
export const jobListingsTable = pgTable(
  "job_listings",
  {
    ...
    sourceId: integer("source_id")
      .notNull()
      .references(() => jobSourcesTable.id, { onDelete: "cascade" }),
  ...
  (table) => [
    unique("job_listings_source_item_unique").on(
      table.sourceKey,
      table.sourceItemId,
    ),
...
export const insertJobListingSchema = createInsertSchema(
  jobListingsTable,
).omit({
  id: true,
  createdAt: true,
 ...
 */
export const trendsCacheTable = pgTable(
  "trends_cache",
  {
    id: serial("id").primaryKey(),
