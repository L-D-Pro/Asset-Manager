...
function jobContainsTerm(jobTerms: string[], filterTerm: string): boolean {
  ...
  return jobTerms.some((t) => normalize(t).includes(normFilter) || normFilter.includes(normalize(t)));
}
...
export function scoreJobAgainstProfile(
  ...
  const hardFilters = (roleProfile.hardFilters as Record<string, unknown>) ?? {};
  const softWeights = (roleProfile.softWeights as Record<string, number>) ?? {};
  ...
  const requiredKeywords = (hardFilters["requiredKeywords"] as string[]) ?? [];
  ...
  const blockedKeywords = (hardFilters["blockedKeywords"] as string[]) ?? [];
  ...
  const minSalary = hardFilters["minSalary"] as number | undefined;
  ...
  for (const skill of requiredSkills) {
    ...
    if (!matched && !matchedSkills.some((m) => normalize(m).includes(normalize(skill)))) {
